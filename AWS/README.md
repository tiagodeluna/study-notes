...
| ----- | ----- |
| `ssh -i <.pem file> <instance user>@<instance public DNS>` | Server Admin | Starts the SSH client program that enables secure connection to the SSH server on a remote machine |
| `scp -i <.pem file> <file to copy> <instance user>@<instance public DNS>:~` | Server Admin | Secure Copy. Connects to the server using the SSH private key file (.pem) and copies the file which name is provided |
| `mysql -u <db user> -p<db password> -h <db public DNS (without port)>` | Server Admin | Mysql command to connect with a remote database |
| `sudo tcpdump 'dst port 8080 and tcp[32:4] = 0x47455420'` | Server Admin | Listen to the requests made to an instance |
| `aws configure` | AWS CLI | Define AWS CLI configuration |
| `aws ec2 run-instances --image-id <image id> --count <number of instances> --instance-type <type> --key-name <.pem filename> --security-groups <security group>` | AWS CLI | Generate new EC2 instances |
| `aws elb create-load-balancer --load-balancer-name <name> --listeners "Protocol=HTTP,LoadBalancerPort=80,InstanceProtocol=HTTP,InstancePort=8080" --availability-zones <at least two zones>` | AWS CLI | Create a Elastic Load Balancer |
| `aws elb register-instances-with-load-balancer --load-balancer-name <name> --instances <instances ids>` | AWS CLI | Register instances to be managed by a laod balancer |
